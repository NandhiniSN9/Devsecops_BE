...
@router.get("/overview")
async def get_overview(period: str | None = Query(default=None),overview_service: OverviewService = Depends(get_overview_service)) -> BaseResponse:
    ...
    """
    try:
        overview_data = await overview_service.get_overview(period)
